...
const TimeZoneDisplay = () => {
	...
	useEffect(() => {
		...
		setTimeZones((prevZones: TimeZone[]) => {
			...
			const uniqueZones = Array.from(
				new Set(updatedZones.map((zone) => JSON.stringify(zone))),
	...
	useEffect(() => {
		const updateTimes = () => {
			const newTimes = timeZones.map((tz: { timezone: string }) => {
				...
				const isDaytime =
					(Number(hour) >= 6 && Number(hour) < 18) || Number(hour) === 17;
	...
	const deleteTimeZone = (id: number) => {
		setTimeZones((prevZones: TimeZone[]) => {
			const updatedZones = prevZones.filter(
				(zone: { id: number }) => zone.id !== id,
			);
			if (typeof localStorage !== "undefined") {
	...
	return (
		<div
			className={`flex flex-col h-screen  ${darkMode ? "bg-gray-900" : "bg-gray-100"} transition-colors duration-500`}
		>
				...
				{times.map((tz) => (
					...
					>
						...
						{tz.isDaytime ? (
							...
							<Moon
								className="absolute top-2 right-2 text-blue-200"
								size={18}
							/>
						)}
					</div>
